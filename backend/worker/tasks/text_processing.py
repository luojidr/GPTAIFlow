...
def text_truncation(
    ...
    truncate_start = 0
    if str(truncate_length).isdigit():
        truncate_length = int(truncate_length)
    if not isinstance(truncate_length,int):
        truncate_start, truncate_length = truncate_length.split('-')
        truncate_length = int(truncate_length)
        truncate_start = int(truncate_start)
    ...
    for text in texts:
        if len(text) <= truncate_length:
            outputs.append(text[truncate_start:])
            continue

        if floating_range <= 0:
            outputs.append(text[truncate_start:truncate_length])
            continue
        ...
        for separator in separators:
            ...
            if separator_index > 0:
                truncated_text = text[truncate_start: truncate_length - floating_range + separator_index]
                break
        else:
            truncated_text = text[truncate_start:truncate_length]
