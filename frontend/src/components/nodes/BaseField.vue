...
const emit = defineEmits(['delete', 'update:show','update:display_name'])
...
const handleNameChange = (newName) => {
  ...
  emit('update:name', fieldName.value)
}
...
<template>
  <div :class="['template-item-field', props.type == 'source' ? 'template-item-output-field' : '']" :style="props.style">
    <div class="template-item-field-text">
      <a-typography-text class="title" :editable="{ onChange: handleNameChange }"
              v-model:content=fieldName>
      </a-typography-text>
