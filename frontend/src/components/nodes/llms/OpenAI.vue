  ...
  templateData: {
    ...
    "template": {
      ...
      "ignore_error_rate": {
        ...
        "value": 0.2,
        "password": false,
        "name": "ignore_error_rate",
        "display_name": "ignore_error_rate",
        "type": "float",
        "clear_after_run": true,
        "list": false,
        "field_type": "number"
...
if (!fieldsData.value.ignore_error_rate) {
  // 如果ignore_error_rate不存在，则添加它并设置默认值
  fieldsData.value.ignore_error_rate = {
    required: false,
    placeholder: "",
    show: false,
    multiline: false,
  ...
  <BaseNode :nodeId="id" :title="t('components.nodes.llms.OpenAI.title')" :description="props.data.description"
    ...
    <template #main>
      <a-row type="flex">
        ...
        <a-col :span="24">
          <BaseField id="ignore_error_rate" :name="t('components.nodes.llms.OpenAI.ignore_error_rate')" required type="target"
            v-model:show="fieldsData.ignore_error_rate.show">
            <a-input-number v-model:value="fieldsData.ignore_error_rate.value" :controls="false" style="width: 100%;" />
          </BaseField>
        </a-col>
      </a-row>
    </template>
