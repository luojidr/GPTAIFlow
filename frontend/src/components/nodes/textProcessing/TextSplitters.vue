...
if (!fieldsData.value.chapter) {
  fieldsData.value.chapter = {
    "required": false,
    "placeholder": "",
    "show": false,
    "multiline": true,
  ...
  <BaseNode :nodeId="id" :title="t('components.nodes.textProcessing.TextSplitters.title')"
    ...
    <template #main>
      <a-row type="flex">
        ...
        <!-- <a-col :span="24" v-if="fieldsData.split_method.value == 'chapter'">
          <BaseField id="chapter" :name="t('components.nodes.textProcessing.TextSplitters.chapter')" required
            type="target" v-model:show="fieldsData.chapter.show">
            <a-input style="width: 100%;" class="field-content" v-model:value="fieldsData.chapter.value"
              :placeholder="fieldsData.chapter.placeholder" />
          </BaseField>
        </a-col> -->
      </a-row>
    </template>
