<script setup>
import { onBeforeMount, defineComponent, ref, reactive, computed, nextTick, h } from "vue"
import { useI18n } from 'vue-i18n'
import { useRouter } from "vue-router"
import { message } from 'ant-design-vue'
import { FilterOutlined, NumberOutlined, ControlOutlined, FieldTimeOutlined, PlusOutlined, TagsOutlined, StarOutlined, StarFilled } from '@ant-design/icons-vue'
...
const workflowRecords = reactive({
  columns: [{
    ...
    // onFilter: (value, record) => record.tags.map(item => item.title).indexOf(value) >= 0,
    width: '300px',
  }, {
  ...
  load: async (params) => {
    ...
    userWorkflowsStore.setUserWorkflows(workflowRecords.data)
...
<template>
  ...
  <a-spin :spinning="updating" class="space-container" v-else>
    <a-row justify="space-between" align="middle" :gutter="[16, 16]">
      ...
      <a-col :span="24">
        <!-- <a-space>
          <a-radio-group v-model:value="workflowRecords.selectTag" button-style="solid"
            @change="workflowRecords.selectTagChange">
      ...
      <a-col class="workflow-container" :span="24">
        <a-table :loading="workflowRecords.loading" :columns="workflowRecords.columns"
          ...
          :pagination="workflowRecords.pagination"  @resizeColumn="workflowRecords.handleResizeColumn" @change="workflowRecords.handleTableChange">
          <template #headerCell="{ column }">
            <template v-if="column.key === 'title'">
              <NumberOutlined />
              {{ t('workspace.workflowSpaceMain.workflow_title') }}
              <a-button type="primary" :icon="h(PlusOutlined)" size="small" @click="add" v-if="['ADMIN','ROOT','EDITOR'].includes(getUserRole())">
              </a-button>
          ...
          <template #customFilterIcon="{ filtered }">
            <a-typography-text :style="{ color: filtered ? '#A1B92E' : '#000' }">
              筛选
            </a-typography-text>
            <FilterOutlined  :style="{ color: filtered ? undefined : '#000' }" />
          </template>
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
              <div class="action-container">
                <a-typography-link @click.prevent="openShareWorkflowModal(record.wid)" v-if="['ADMIN','ROOT','EDITOR'].includes(getUserRole())">
