<template>
  <a-modal v-model:open="open" :title="t('components.workspace.shareWorkflowModal.share_workflow')" @ok="share"
    ...
    <a-form ref="formRef" :model="shareData">
      ...
      <a-form-item
        :rules="[{ required: true, min: 10, message: t('components.workspace.shareWorkflowModal.brief_min_require', { count: 10 }), trigger: 'blur' }]"
        name="brief">
        <template #label>
        ...
        <MarkdownEditor v-model="shareData.brief" :key="templateWid" />
      </a-form-item>
  ...
  try {
    ...
    if (shareResponse.status == 200) {
      Modal.success({
        ...
        content: t('components.workspace.shareWorkflowModal.share_success_brief'),
      })
    } else {
