...
import { FullScreenOne, OffScreenOne, Edit, Lightning, Outgoing } from '@icon-park/vue-next'
...
import { BarsOutlined, EditTwoTone } from '@ant-design/icons-vue';
...
const runWorkflow = async () => {
  ...
  try {
    inputFields.value.forEach((field) => {
      ...
      if (field.field_type == 'checkbox' && field.value.length != 0 ) {
        return
      }
      if (!field.show) {
        return
      ...
      if (field.field_type == 'number' && typeof field.value != 'number') {
        ...
        currentFieldValid = false
      } else if (field.field_type == 'checkbox' && field.value.length == 0) {
        currentFieldValid = false
      }
...
const setWorkflowRecord = (record) => {
  ...
  errorDetail.value = record.data.error_detail
...
const websiteChange = (value) => {
  inputFields.value.find(item => item.fieldName === 'rankOptions').vShow = value == "番茄小说";
  inputFields.value.find(item => item.fieldName === 'qdSexOptions').vShow = value == "起点小说";
  inputFields.value.find(item => item.fieldName === 'dbListType').vShow = value == "豆瓣小说";
  if (value != "番茄小说") {
    inputFields.value.find(item => item.fieldName === 'rankOptions').value = []
  } else if (value != "起点小说") {
    inputFields.value.find(item => item.fieldName === 'qdSexOptions').value = []
  } else if (value != "豆瓣小说") {
    inputFields.value.find(item => item.fieldName === 'dbListType').value = []
  }
} 

const handleUploadSuccess = () => {
  inputFields.value.find(item => item.fieldName === 'prompt').disabled = true
  inputFields.value.find(item => item.fieldName === 'prompt').value = ""
}

const handleRemoveSuccess = () => {
  inputFields.value.find(item => item.fieldName === 'prompt').disabled = false
  inputFields.value.find(item => item.fieldName === 'prompt').value = ""
}
</script>
  ...
  <a-spin :spinning="updating" class="space-container" v-else>
    ...
    <a-row justify="space-between" align="middle">
      ...
      <a-col>
        <a-space>
          ...
          <a-button @click="openEditor" v-if="['ADMIN','ROOT','EDITOR'].includes(getUserRole())">
    ...
    <a-row :gutter="[16, 16]">
        ...
        <a-alert :type="alertType" show-icon>
          <template #message>
            ...
            <span v-if="recordStatus == 'FAILED' && recordErrorTask != '.'">
             ...
             <a-divider type="vertical" />
              {{ t('workspace.workflowSpace.record_error_detail', {
                detail: errorDetail
              }) }}
            </span>
          </template>
      ...
      <a-col :xxl="6" :xl="8" :lg="10" :md="24" v-show="!outputMaximized">
        <a-row :gutter="[16, 16]">
          <a-typography-title :level="3">
            <EditTwoTone twoToneColor="#28c5e5"/>
            {{ t('workspace.workflowSpace.inputs') }}
          </a-typography-title>
          <a-col :span="24">
            <a-form layout="vertical">
              <div v-for="( field, fieldIndex ) in inputFields " :key="`field-${field}-${fieldIndex}`">
                <a-form-item v-if="!nonFormItemsTypes.includes(field.field_type) && (field.vShow !== undefined ? field.vShow : true)">
                  <template #label>
                    <span v-if="field.vShow !== undefined ? field.vShow : true">{{ field.display_name }}</span>
                  </template>
                  <TemperatureInput v-model="field.value" v-if="field.category == 'llms' && field == 'temperature'" />
                  <a-select v-model:value="field.value" :options="field.options" @change="value => websiteChange(value, fieldIndex)"
                    v-else-if="field.field_type == 'select' && field.name == 'websiteType'" />
                  <a-select v-model:value="field.value" :options="field.options"
                    v-else-if="field.field_type == 'select' && field.name" />
                  <a-cascader v-model:value="field.value" :options="field.options"
                    v-else-if="field.field_type == 'cascader' && field.vShow" />
                  <a-textarea v-model:value="field.value" :autoSize="true" :showCount="true"
                    :placeholder="field.placeholder" :disabled="field.disabled" v-else-if="field.field_type == 'textarea'" />
                  <a-input v-model:value="field.value" :placeholder="field.placeholder"
                    v-else-if="field.field_type == 'input'" />
                  <a-input-number v-model:value="field.value" :placeholder="field.placeholder" :step="0.1"
                    v-else-if="field.field_type == 'number'" />
                  <a-checkbox-group v-model:value="field.value"  v-else-if="field.field_type == 'checkbox'">
                    <a-checkbox v-for="(option, key) in field.options" :key="key" :label="option.label" :value="option.value">{{ option.label }}</a-checkbox>
                  </a-checkbox-group>
                  <UploaderFieldUse v-model="field.value" v-else-if="field.field_type == 'file'" @upload-success="() => handleUploadSuccess()" @remove-success="() => handleRemoveSuccess()" />
      ...
      <a-col :xxl="outputMaximized ? 24 : 18" :xl="outputMaximized ? 24 : 16" :lg="outputMaximized ? 24 : 14" :md="24">
        <a-typography-title :level="3" style="display: flex; justify-content: space-between;">
          <span>
            <Outgoing theme="outline" fill="#28c5e5"/>
          ...
          <a-row :gutter="[16, 16]">
            <a-col :span="24" class="draggable-item" v-for="(node, index) in outputNodes"
              ...
              <div v-else-if="node.type == 'Table'">
                <TableRenderer :output="node.data.template.output.value" style="width: 100%;min-height: 50vh;" />
              </div>
