...
import { getUserId,getUserRole } from '@/utils/user' 
...
onBeforeMount(async () => {
  const response = await workflowTemplateAPI('get', { tid: workflowTemplateId })
...
const addTemplateToUserWorkflows = async () => {
  ...
  workflow.wid = response.data.wid
...
const deleteWorkflowTemplate = async () => {
  const response = await workflowTemplateAPI('delete', { tid: workflowTemplateId })
  if (response.status == 200) {
    message.success(t('workspace.workflowSpace.delete_success'))
    router.push({ name: 'WorkflowSpaceMain', params:{tab:'official-workflow-templates'} })
  } else {
...
const openEditor = () => {
  router.push({ name: 'WorkflowEditor', params: { workflowId: templateData.value.wid } })
}
    ...
    <a-row justify="space-around">
      <a-col :lg="12" :md="12" :sm="24" :xs="24">
        ...
        <a-space>
          ...
          <template v-if="templateData.user.user_id === getUserId().replaceAll('-','') || ['ADMIN','ROOT'].includes(getUserRole())">
            ...
            <a-popconfirm placement="leftTop" :title="t('workspace.workflowSpace.delete_confirm')"
              @confirm="deleteWorkflowTemplate">
